...
import myapp.hoang.core.config.NetworkConfig.ROUTE_SIGNUP_CHECK_VERIFICATION_CODE
import myapp.hoang.core.config.NetworkConfig.ROUTE_SIGNUP_VERIFY_VERIFICATION_CODE
...
class KtorSignupService @Inject constructor(
    ...
    override suspend fun sendVerificationCode(mobileNumber: String): VerificationResponse {
        return client.get {
            url(ROUTE_SIGNUP_CHECK_VERIFICATION_CODE)
    ...
    override suspend fun checkVerificationCode(mobileNumber: String, code: String): VerificationCheckResponse {
        return client.get {
            url(ROUTE_SIGNUP_VERIFY_VERIFICATION_CODE)
            parameter("mobile-number", mobileNumber)
            parameter("code", code)
        }.body()
    }
